function [xs, us, ts, extraout] = rollout_time_varying_controller( ...
...
while ~end_simulation
    ...
    if t == t0 && ~isempty(u0)
        ...
        [~, extra_t] = controller(t, x, 'verbose', 0);
    ...
    elseif t == t0 && ~isempty(mu0)
        ...
        elseif ref_type == 1
            ...
            [u, extra_t] = controller(t, x, 'u_ref', u_ref_t, ...
                'with_slack', with_slack, 'verbose', (verbose_level>=2));                        
        elseif ref_type == 2
            ...
            [u, extra_t] = controller(t, x, 'mu_ref', mu_ref_t, ...
                'with_slack', with_slack, 'verbose', (verbose_level>=2));                        
        end
    end
    if verbose_level >= 1
